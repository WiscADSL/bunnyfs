...
class InMemInode {
 public:
  ...
  // #fs_req still in progress (used for migration)
  uint32_t num_fs_req_in_prog{0};

  // lease info
  std::set<pid_t> leased_app;

  bool fetchSetDirty(bool d) {
...
#endif
  std::vector<ExportedBlockBufferItem> block_buffers;
...
class FsImpl {
 public:
  ...
  FsImpl(FsProcWorker *worker, const char *memPtr, float dataDirtyFlushRatio,
         int numPartitions);
  ...
  FsImpl(FsProcWorker *worker, const char *bmapMemPtr, char *dataBlockMemPtr,
  ...
  FsImpl(FsProcWorker *worker, const char *bmapMemPtr,
  ...
  static char *getDataBlockMemPtrPartition(char *dataBlockMemPtr,
    ...
    SPDLOG_DEBUG("dataBlockMemPtr:{:p} idx:{} pttNumBlock:{} ret:{:p}",
  ...
  void adjustCacheSize(sched::Tag t) { dataBlockBuf_->adjustCacheSize(t); }

 private:
  int idx_{-1};
  FsProcWorker *fsWorker_{nullptr};
  ...
  static constexpr int kBufferSlowLowWatermark = 5000;
  ...
  BlockBufferHandle getBlockForIndex(BlockBuffer *blockbuf, uint32_t blockNo,
                                     FsReq *fsReq, uint32_t index);
  BlockBufferHandle getBlockForIndex(BlockBuffer *blockBuf, uint32_t blockNo,
                                     FsReq *fsReq, bool doSubmit,
                                     bool doBlockSubmit, bool &canOverwritten,
                                     uint32_t index);
  BlockBufferHandle getBlock(BlockBuffer *blockBuf, uint32_t blockNo,
                             FsReq *fsReq);
  BlockBufferHandle getBlock(BlockBuffer *blockBuf, uint32_t blockNo,
                             FsReq *fsReq, bool doSubmit, bool doBlockSubmit,
                             bool &canOverwritten, uint32_t new_index = 0);
  BlockBufferHandle getLockedDirtyBitmap(cfs_bno_t bitmap_blockno);
  void releaseLockedDirtyBitmap(BlockBufferHandle buf);
...
inline BlockBufferHandle FsImpl::getLockedDirtyBitmap(
    cfs_bno_t bitmap_blockno) {
...
inline void FsImpl::releaseLockedDirtyBitmap(BlockBufferHandle buf) {

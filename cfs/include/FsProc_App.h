...
class AppProc {
 public:
  ...
  AppProc(FsProcWorker *worker, int aid, int shmBaseOffset, AppCredential &cred,
          uint32_t cache_size, int64_t bandwidth, int64_t cpu_cycles);

  ~AppProc();

  void initShm(int aid, int shmBaseOffset);
  ...
  FsProcWorker *getWorker() { return worker; }
  ...
  auto GetInos() {
    ...
    for (auto &elem : accessed_ino)
      inos.insert(elem.ino);
    return inos;
  }
  auto GetTidsForIno(cfs_ino_t ino) {
    std::vector<int> result;
    for (auto &elem : accessed_ino) {
      if (elem.ino == ino) result.push_back(elem.tid);
    }
    return result;
  }
  auto GetAccessedInoAndErase(int tid) {
    std::unordered_set<cfs_ino_t> erased_ino;
    for (auto it = accessed_ino.begin(); it != accessed_ino.end();) {
      if (it->tid == tid) {
        erased_ino.insert(it->ino);
        it = accessed_ino.erase(it);
      } else {
        it++;
      }
    }
  ...
  void AccessIno(int tid, cfs_ino_t ino) {
    accessed_ino.emplace(tid, ino);
  }
  void EraseIno(int tid, cfs_ino_t ino) {
    accessed_ino.erase({tid, ino});
  }
  void EraseIno(cfs_ino_t ino) {
    std::erase_if(accessed_ino,
                  [ino](const auto &elem) { return elem.ino == ino; });
  }
 ...
 private:
  // worker id associated with this app
  FsProcWorker* worker;
  int aid;
  AppCredential cred;
  ...
  struct InoWithTid {
    ...
    bool operator==(const InoWithTid &other) const {
      return tid == other.tid && ino == other.ino;
    }

    struct Hash {
      std::size_t operator()(const InoWithTid &e) const {
        return std::hash<int>()(e.tid) ^ std::hash<cfs_ino_t>()(e.ino);
      }
    };
  };

  // used for redirect inodes
  std::unordered_set<InoWithTid, InoWithTid::Hash> accessed_ino;
  // tid, cycles
...
#ifdef DO_SCHED
  // this is an encapsulation for scheduling subsystem
  sched::Tenant tenant;

 public:
  sched::Tenant &getTenant() { return tenant; }
#endif
};

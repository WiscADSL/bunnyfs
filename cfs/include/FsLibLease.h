...
class OpenLease {
 ...
 public:
  OpenLease(int basefd, const std::string& standard_path, uint64_t file_size)
    ...
    lease_start = PlatformLab::PerfUtils::Cycles::rdtscp();
  }
  ...
  bool Expired(uint64_t current) {
    return PlatformLab::PerfUtils::Cycles::toMicroseconds(
               current - lease_start) > lease_term;
  }
 ...
 public:
  ...
  void* GetData(FsLibMemMng* mem, size_t* asize, size_t* fsize) {
    ...
    for (int i = 0; i < data_array_size; ++i) {
      if (data_array[i].first == nullptr) {
        throw std::runtime_error(
            "data_array[i] is nullptr. Potential memory leak");
      }

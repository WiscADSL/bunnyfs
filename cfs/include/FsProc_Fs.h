...
static void inline SpinSleep(uint64_t ns) {
  uint64_t cur_ts = 0;
  uint64_t start_ts = PlatformLab::PerfUtils::Cycles::rdtsc();
  volatile int waitNum = 0;
  while (true) {
    cur_ts = PlatformLab::PerfUtils::Cycles::rdtsc();
    // c++20 deprecate ‘++’ expression of ‘volatile’
    waitNum = waitNum + 1;
    if (PlatformLab::PerfUtils::Cycles::toNanoseconds(cur_ts - start_ts) > ns) {
      break;
    }
  }
}
...
class GlobalAppCredTable {
 public:
  ...
  void AddAppCredential(struct ucred &cred, int idx) {
    assert(idx >= 0);
    auto app_cred = new AppCredential(cred.pid, cred.gid, cred.uid);
...
class FsProc {
 public:
  ...
  FsProc(int tNum, int appNum, const char *readyFname, const char *exitFname);
  FsProc(bool doInline);
  ~FsProc() {}

  int getNumThreads() { return numThreads; }

  int getNumApps() { return numAppProc; }
  ...
  // @param shmOffsetVec
  // @param workerAppConfigs: worker-app pair configurations
 ...
 private:
  ...
  // this function should only be called after all workers have initialized
  // their tenants
 public:
  void startAllocator();
  // for now we don't stop allocator but just directly exit the process
#endif
};
...
class FsProcWorker {
 public:
  ...
  AppProc *GetApp(pid_t pid) const {
  ...
  const std::unordered_map<pid_t, AppProc *> &GetAppMap() { return appMap; }

  template <typename Fn>
  void forEachApp(Fn &&fn) const {
    for (auto a : appList) fn(a);
  }
  ...
  void ProcessSchedNewResrcAlloc(sched::AllocDecision *decision);
  void schedMigrateInode(AppProc *app,
                         const std::vector<std::tuple<int, int>> &inode_move);

  int CheckFutureRoutingOnReqCompletion(AppProc *app, int tid,
                                        InMemInode *inode);

  // initialize apps based on appConfigs
  void InitApps();
...
#else  // DO_SCHED
  virtual void submitReadyReq(FsReq *req) {
    AppProc *app = req->getApp();
    if (app) {
      app->getTenant().add_intl_queue(req);
    }
  }
#endif
  ...
  // do actual submission to device (called by submitAsyncReadDevReq() after
  // updating submission-related data structures)
  virtual int doSubmitAsyncRead(FsReq *fsReq, BlockReq *blockReq);
  ...
  *****************************************************************************/
  ...
  virtual bool checkInoFsyncToWait(cfs_ino_t ino) {
    ...
    bool rt = false;
    if (inoHasActiveAppFsyncSet.size() >
            blockbuffer::Flusher::kNumFgFlushLimit &&
 ...
 protected:
...
#ifdef DO_SCHED
  // when start inner loop for the first time, reset cpu progress
  uint64_t cpu_prog_epoch_ts = 0;
#else
  ...
  std::vector<AppProc *> appList;  // to speed up traversal
...
#else  // DO_SCHED
  bool processRecvReadyQueue(sched::Tenant &t);
  bool processInternalReadyQueue(sched::Tenant &t);
  uint64_t processBlockReadyQueue(sched::Tenant &t);
#endif
...
class FsProcWorkerMaster : public FsProcWorker {
 ...
 private:
  ...
  std::vector<NewedInodes> pending_newed_inodes;
...
inline size_t FsProcWorker::NumInProgressRequests(cfs_ino_t ino) {
  InMemInode *inodePtr = fileManager->GetInMemInode(ino);
  return inodePtr ? inodePtr->num_fs_req_in_prog : 0;
}

...
class BlockReq {
 public:
  ...
  BlockReq(uint64_t blockNo, BlockBufferHandle bufIterm, char *dst,
           enum FsBlockReqType t) {
    init(blockNo, bufIterm, dst, t);
  }

  void init(uint64_t blockNo, BlockBufferHandle bufIterm, char *dst,
            enum FsBlockReqType t);
  void clear() {
    if (secondary_fs_reqs) delete secondary_fs_reqs;
  }

  enum FsBlockReqType getReqType() { return reqType; }
  uint64_t getBlockNo() { return blockNo; }
  char *getBufPtr() { return memPtr; }
  BlockBufferHandle getBufferItem();
  void setBlockNoSeqNo(uint64_t seqNo);
  uint64_t getBlockNoSeqNo();
  bool getSubmitted();
  void setSubmitted(bool s);

  FsReq *get_primary_fs_req() { return primary_fs_req; }
  std::vector<FsReq *> *get_secondary_fs_reqs() { return secondary_fs_reqs; }
  void add_fs_req(FsReq *fs_req) {
    if (!primary_fs_req) {
      primary_fs_req = fs_req;
    } else {
      if (!secondary_fs_reqs) secondary_fs_reqs = new std::vector<FsReq *>();
      secondary_fs_reqs->emplace_back(fs_req);
    }
  }

  void set_submitted_ts() {
    assert(submitted_ts == 0);
    submitted_ts = PlatformLab::PerfUtils::Cycles::rdtsc();
  }
  [[nodiscard]] uint64_t get_latency() {
    assert(submitted_ts);
    return PlatformLab::PerfUtils::Cycles::rdtsc() - submitted_ts;
  }

  void set_buf_flush_req(BufferFlushReq* buf_flush_req) {
    buf_flush_req_ = buf_flush_req;
  }

  [[nodiscard]] BufferFlushReq* get_buf_flush_req() const {
    return buf_flush_req_;
  }
 ...
 private:
  ...
  char *memPtr;
  BlockBufferHandle bufferItem;
  FsBlockReqType reqType;

  uint64_t submitted_ts;

  // the fs_req that initiates the block read is the primary
  // in the meanwhile, if there is a concurrent fs_req trying to read the same
  // block, will be considered as the secondary fs_reqs
  FsReq *primary_fs_req = nullptr;
  std::vector<FsReq *> *secondary_fs_reqs = nullptr;

  BufferFlushReq* buf_flush_req_ = nullptr;
};
...
class FsReqIoUnitHelper {
 public:
  ...
  BlockReq *addIoUnitSubmitReq(cfs_bno_t iouNo, BlockBufferHandle item,
...
class FsReq {
 ...
 public:
  ...
  /*****************************************************************************
  void addBlockSubmitReq(uint32_t blockNo, BlockBufferHandle bufItem, char
  *dst,
                         FsBlockReqType type);
  void addSectorSubmitReq(uint32_t sectorNo, BlockBufferHandle bufItem,
                          char *dst, FsBlockReqType type);
  // void addBlockWriteReq(uint32_t blockNo, BlockBufferHandle bufItem, char
  // *src);
  void addSectorWriteReq(uint32_t sectorNo, BlockBufferHandle bufItem,
                         char *src);
  ...
  *****************************************************************************/
  ...
  void inc_pending() { ++num_pending; }
  void dec_pending() { --num_pending; }
...
#endif
      curOnCpuTermStartTick = PlatformLab::PerfUtils::Cycles::rdtsc();
    }
  }
  ...
  void stopOnCpuTimer() {
    if (ifUseOnCpuTimer()) {
      ...
      perfstat_cycle_t diff =
          PlatformLab::PerfUtils::Cycles::rdtsc() - curOnCpuTermStartTick;
      if (diff > 0) {
        totalOnCpuCycles += diff;
#ifdef DO_SCHED
        if (tenant != nullptr) tenant->record_cpu_consump(diff);
#endif
      }
#ifdef FSREQ_LOCAL_TIMER_DBG
...
#undef FSREQ_LOCAL_TIMER_DBG
  ...
  static perfstat_cycle_t genTickCycle() {
    return PlatformLab::PerfUtils::Cycles::rdtscp();
  }
  // static perfstat_cycle_t genTickCycle() { return
  // PlatformLab::PerfUtils::Cycles::rdtsc();
  // }
  ...
  *****************************************************************************/
  ...
  /*****************************************************************************
  void initBioHelpers() {
    blockIoHelper = new FsReqIoUnitHelper(&blockPendingMap,
    &submitBlockReqMap,
                                          (BSIZE), false);
 ...
 private:
  InMemInode *inode_in_prog{nullptr};

 public:
  void set_inode_in_prog(InMemInode *n) { inode_in_prog = n; }
  InMemInode *get_inode_in_prog() { return inode_in_prog; }
 ...
 public:
  sched::Tenant *get_tenant() { return tenant; }
  void set_tenant(sched::Tenant *t) { tenant = t; }
#endif
...
class BlockReqPool {
  constexpr static int kPerWorkerBlockReqPoolCapacity = 65536;
 ...
 private:
  BlockReq *pool_;
  std::vector<BlockReq *> block_req_queue_;
};
...
class BufferFlushReq {
 public:
  ...
  uint64_t submitTs = 0;
  uint64_t doneTs = 0;
  static uint64_t tapFlushTs() {
    return PlatformLab::PerfUtils::Cycles::rdtsc();
  }
 ...
 private:
  ...
  std::list<BlockBufferHandle> toSubmitFlushBlocks;

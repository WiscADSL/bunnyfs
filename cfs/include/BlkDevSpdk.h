...
class BlkDevSpdk : public BlkDev {
 public:
  ...
  // inherited functions
  virtual int devInit();
  virtual int read(uint64_t blockNo, char *data, void *ctx_payload);
  ...
  virtual int readSector(uint64_t sectorNo, char *data, void *ctx_payload);
 ...
 private:
  ...
  std::vector<std::vector<bdev_reqid_t>> tidUnusedReqidList;
...
class BlkDevPosix : public BlkDevSpdk {
 public:
  ...
  // inherited functions
  int devInit();
  int read(uint64_t blockNo, char *data, void *ctx_payload = nullptr);
  ...
  int readSector(uint64_t sectNo, char *data, void *ctx_payload = nullptr);

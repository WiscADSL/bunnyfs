...
FsImpl::FsImpl(FsProcWorker *worker, const char *memPtr,
...
FsImpl::FsImpl(FsProcWorker *worker, const char *bmapMemPtr,
...
cfs_dirent *FsImpl::lookupDirDentry(FsReq *fsReq, InMemInode *dirInode,
  ...
  // uint32_t fileIno = 0;
  BlockBufferHandle itemPtr = nullptr;
  ...
  if (dentryBlockNo > 0) {
    ...
    if (itemPtr->isInMem()) {
      ...
      if ((direntPtr + inoBlockDentryIdx)->inum > 0 &&
        ...
        dataBlockBuf_->setBlockDirty(itemPtr, dirInode->i_no);
        // release the block
        dataBlockBuf_->releaseBlock(itemPtr);
      } else {
...
int FsImpl::writeFileInode(FsReq *req, uint32_t inodeNo,
  ...
  if (it != inodeMap_.end()) {
    SPDLOG_DEBUG("writeFileInode - inodeNo:{} update inodeMap idx_:{}", inodeNo,
                 idx_);
    // inodeMap_[inodeNo]->fetchSetDirty(true);
    it->second->fetchSetDirty(true);
    dirtyInodeSet_.emplace(inodeNo);
    return 1;
  } else {
  ...
  BlockBufferHandle itemPtr =
      getBlock(inodeSectorBuf_, ino2SectorNo(inodeNo), req);
  if (itemPtr->isInMem()) {
    // FIXME (jingliu): do we still need to set this item to dirty? InMemInode
    // has this info.
    inodeSectorBuf_->setBlockDirty(itemPtr, 0);
    //    inodeSectorBuf_->releaseBlock(itemPtr); // TODO: proper release
    return 1;
  }
...
int64_t FsImpl::readInodeToUCache(
  ...
  for (uint i = 0; i < pagesToRead.size(); i++) {
    ...
    BlockBufferHandle itemPtr = getBlockForIndex(
...
int64_t FsImpl::writeEntireInode(FsReq *req, InMemInode *inode,
  ...
  for (tot = 0; tot < realBytes; tot += m, off += m, srcPtr += m) {
    ...
    if (doWrite) {
      ...
      dataBlockBuf_->setBlockDirty(itemPtr, inode->i_no);
...
int64_t FsImpl::checkAndFlushDirty(FsProcWorker *worker) {
  ...
  // flush DataBuffer
  if (dataBlockBuf_->flusher.checkIfNeedBgFlush()) {
    ...
    if (numFlushed == 0) {
      dataBlockBuf_->flusher.doFlushDone();
    }
  } else {
    numFlushed = -1;
  }
...
int64_t FsImpl::flushInodeData(FsProcWorker *worker, FsReq *req,
  ...
  needFlush = true;
  int64_t numFlushed = 0;
  if (!dataBlockBuf_->flusher.checkIfFgFlushReachLimit()) {
    if (dataBlockBuf_->flusher.checkIfIdxFgFlushInflight(req->getFileInum())) {
      numFlushed = -1;
      return numFlushed;
    }
    auto bufFlushReq = new BufferFlushReq(dataBlockBuf_, worker);
    ...
    SPDLOG_DEBUG("numToFlush:{}", numToFlush);
    if (numToFlush > 0) {
      dataBlockBuf_->flusher.addFgFlushInflightNum(1);
      numFlushed = bufFlushReq->submitFlushReqs();
      ...
      if (numFlushed == 0) {
        ...
        SPDLOG_DEBUG("numFlushed:{} for ino {}", numFlushed,
                     req->getFileInum());
        dataBlockBuf_->flusher.addFgFlushWaitIndex(req->getFileInum());
      }
...
InMemInode *FsImpl::BlockingGetInode(cfs_ino_t ino, bool &io_done,
  ...
  [[maybe_unused]] auto it = inodeMap_.find(ino);
  ...
  auto item = inodeSectorBuf_->getBlock(ino2SectorNo(ino));
  if (item == nullptr) {
    std::runtime_error("Fatal, cannot getBlock for BlockingGetInode");
  }
  if (item->isInMem()) {
    io_done = false;
    return nullptr;
  ...
  item->set_IO_done();
  // NOTE: the cached block should be released when the inode is freed.
  //  but since the lifetime of the inode (usually root inode) is poorly managed
...
InMemInode *FsImpl::getInode(uint32_t ino, FsReq *fsReq, bool doSubmit) {
  ...
  BlockBufferHandle bufferItemPtr = nullptr;
#ifndef NONE_MT_LOCK
  ...
  if (it != inodeMap_.end() && (it->second)->isInodeValid()) {
    ...
    if (doSubmit) {
      ...
      bufferItemPtr->set_IO_done();
      // TODO: the cached block should be released when the inode is freed.
    }
...
BlockBufferHandle FsImpl::getBlockForIndex(BlockBuffer *blockbuf,
...
BlockBufferHandle FsImpl::getBlock(BlockBuffer *blockBuf, uint32_t blockNo,
                                   FsReq *fsReq) {
  bool needSubmit = false;
...
BlockBufferHandle FsImpl::getBlock(BlockBuffer *blockBuf, uint32_t blockNo,
  ...
  SPDLOG_DEBUG("FsImpl::getBlock() blockSize:{} blockNo:{}",
               blockBuf->blockSize, blockNo);

  sched::Tenant *tenant = nullptr;
#ifdef DO_SCHED
  tenant = fsReq ? fsReq->get_tenant() : nullptr;
#endif
  BlockBufferHandle item =
      blockBuf->getBlock(blockNo, new_index, tenant, /*is_write*/ !doSubmit);
  if (item == nullptr) {
    if (tenant == nullptr) {
      SPDLOG_ERROR(
          "FsImpl::getBlock() blockBuf->getBlock({}, {}, {}) failed for {}",
          blockNo, new_index, fmt::ptr(tenant), blockBuf->bufferName);
    } else {
      SPDLOG_ERROR(
          "FsImpl::getBlock() blockBuf->getBlock({}, {}, {}) failed for {}",
          blockNo, new_index, tenant->get_app()->getAid(),
          blockBuf->bufferName);
    }
  ...
  BlockReq *pending_blk_req = item->getPendingBlkReq();
  if (pending_blk_req) {
    assert(pending_blk_req->get_primary_fs_req());
    pending_blk_req->add_fs_req(fsReq);
    fsReq->inc_pending();
    /* No long add to FsReq's queue*/
    // if (blockBuf->blockSize == (SSD_SEC_SIZE)) {
    //   fsReq->addSectorWaitReq(blockNo);
    // } else {
    //   fsReq->addBlockWaitReq(blockNo);
    // }
    blockBuf->releaseBlock(item);
  ...
  if (doSubmit) {
    ...
    assert(!item->getPendingBlkReq());
    BlockReq *blk_req = fsWorker_->block_req_pool.alloc();
    blk_req->init(blockNo, item, item->getBufPtr(), curType);
    item->set_IO_submitted(blk_req);
    blk_req->add_fs_req(fsReq);
    fsReq->inc_pending();
    assert(fsReq == blk_req->get_primary_fs_req());
    fsWorker_->submitAsyncReadDevReq(fsReq, blk_req);
  } else {
    canOverwritten = true;
  }
...
void FsImpl::initMemAddrBmapBlockBuf() {
  SPDLOG_DEBUG("initMemAddrBmapBlockBuf called for idx:{} bmapPtr:{}", idx_,
               (void *)bmapBlockBuf_);
  ...
  SPDLOG_DEBUG(
      "initBmapBuf - bmapMemPtr_:{}, numBmapOnDiskBlocksTotal_:{}, "
      "NMEM_BMAP_BLK:{}, numPartitions_:{}, idx_:{}, pttBmapNumBlocks:{}",
      (void *)bmapMemPtr_, numBmapOnDiskBlocksTotal_, NMEM_BMAP_BLOCK,
      numPartitions_, idx_, pttBmapNumBlocks);
  bmapBlockBuf_ = new BlockBuffer(pttBmapNumBlocks, BSIZE, bmapBlkMemPtr, false,
                                  std::string("Bmap-") + std::to_string(idx_));
}
...
void FsImpl::initMemAddrInodeSectorBuf() {
  ...
  if (idx_ == 0) {
    ...
    SPDLOG_DEBUG(
        "initInodeBuf - inodeSectorMemPtr_:{}, idx_:{}, pttInodeNumSectors:{}",
        (void *)inodeSectorMemPtr_, idx_, pttInodeNumSectors);
    inodeSectorBuf_ =
        new BlockBuffer(pttInodeNumSectors, ISEC_SIZE, inodeSecMemPtr, false,
                        std::string("Inode-") + std::to_string(idx_));
  } else {
...
#ifdef DO_SCHED
  block_no_t totalNumBlocks = 0;
  std::vector<std::pair<sched::Tag, size_t>> config;
  fsWorker_->forEachApp([&](AppProc *app) {
    auto &tenant = app->getTenant();
    uint32_t numBlocks = tenant.get_max_cache_size();
    if (sched::params::policy::cache_partition)
      config.emplace_back(sched::Tag{.tenant = &tenant}, numBlocks);
    totalNumBlocks += numBlocks;
  });

  if (totalNumBlocks > pttDataNumBlocks) {
    SPDLOG_ERROR("totalNumBlocks = {}, pttDataNumBlocks = {}", totalNumBlocks,
                 pttDataNumBlocks);
    throw std::runtime_error("totalNumBlocks > pttDataNumBlocks");
  }

  if (!sched::params::policy::cache_partition)
    config.emplace_back(sched::tag::global, totalNumBlocks);

  // tag==unalloc indicates free buffer item
  config.emplace_back(sched::tag::unalloc, pttDataNumBlocks - totalNumBlocks);

  dataBlockBuf_ =
      new BlockBuffer(config, BSIZE, dataBlkMemPtr, false, curBufferName);
#else
  dataBlockBuf_ = new BlockBuffer(pttDataNumBlocks, BSIZE, dataBlkMemPtr, false,
                                  curBufferName);
#endif
  // Actively flushing
  dataBlockBuf_->flusher.setDirtyRatio(dataBlockBufDirtyFlushRato_);
  dataBlockBuf_->flusher.setDirtyFlushOneTimeSubmitNum(6);
  // dataBlockBuf_->setIfReportStats(true);
  SPDLOG_DEBUG("Data Block Flusher: {}", dataBlockBuf_->flusher);
}
...
void FsImpl::_fsExitFlushSingleBlockBuffer(FsProcWorker *procHandler,
                                           ...
                                           bool dbg) {
  SPDLOG_INFO("Buffer-{} has {} dirty blocks. wid:{}", blkbuf->bufferName,
              blkbuf->flusher.getDirtyItemNum(), procHandler->getWid());
  int rc;
  int writeCnt = 0;

  blkbuf->forEachBlock([&](block_no_t curBlockNo, BlockBufferHandle curItem) {
    if (dbg) {
      fprintf(stderr,
              "===>flush Single Buffer blockno:%u wid:%d blockDirty?:%d\n",
              curBlockNo, procHandler->getWid(), curItem->isDirty());
    }
    assert(curItem.get_key() == curBlockNo);
    if (forceFlush || curItem->isDirty()) {
      writeCnt++;
  ...
  SPDLOG_INFO("Buffer-{} exit flushing do disk write {} blocks",
              blkbuf->bufferName, writeCnt);
  procHandler->writeToWorkerLog(blkbuf->bufferName +
...
void FsImpl::_fsExitFlushSingleSectorBuffer(FsProcWorker *procHandler,
  ...
  int rc;
  blkbuf->forEachBlock([&](block_no_t curSectorNo, BlockBufferHandle curItem) {
    assert(curItem.get_key() == curSectorNo);
...
void FsImpl::uninstallInode(InMemInode *inode) {
  ...
  inodeMap_.erase(inode->i_no);  // TODO: should free the inode and release
                                 // the buffer
...
void FsImpl::splitInodeDataBlockBufferSlot(
    InMemInode *inode, std::vector<ExportedBlockBufferItem> &items) {
...
void FsImpl::installInodeDataBlockBufferSlot(
    InMemInode *inode, const std::vector<ExportedBlockBufferItem> &items) {
...
#define DUMP_PACK_INMEMINODE_ATTR_TO_JSON(j, inode, attr) \
  ...
  if (dumpf.is_open()) {
    ...
    bmapBlockBuf_->forEachBlock(
        [&](block_no_t curBlockNo, BlockBufferHandle curItem) {
          std::string curF = bmapName.c_str() + std::to_string(curBlockNo);

...
static void EmbedThreadIdToAsOpRet(T &ret_field) {
  ...
  ret_field = static_cast<T>(threadFsTid);
}
...
void FsService::initService() {
  ...
  char shmfile[128];
  snprintf(shmfile, 128, "/shmipc_mgr_%08d", (int)shmKey);
    ...
    if (rc > 0 && size != 0) {
      {
        ...
        [[maybe_unused]] auto retval =
            gLibSharedContext->pathOpenLeaseMap.emplace(standardPath,
                                                        new_entry);
        assert(retval.second);
        [[maybe_unused]] auto retval2 =
            gLibSharedContext->fdOpenLeaseMap.emplace(rc, new_entry);
...
ssize_t fs_uc_pread(int fd, void *buf, size_t count, off_t offset) {
  ...
  fhPtr->lock_.unlock();
  // deal with lease
  [[maybe_unused]] auto widIt = gLibSharedContext->fdWidMap.find(fd);
  assert(widIt != gLibSharedContext->fdWidMap.end());
  int retnr = -1;
  int wid = -1;
  if (success) {
...
int fs_allocated_pread_submit_internal(async_ctx_rw *ctx) {
  int wid = -1;
  FsService *service = getFsServiceForFD(ctx->fd, wid);
  ctx->service = service;
  ...
  // fill the shmName and dataPtr offset
  int err = 0;
  threadMemBuf->getBufOwnerInfo(ctx->buf, false, shmid, dataPtrId, err);
  if (err) {
    fprintf(stderr, "fs_allocated_pread_internal: Error in getBufOwnerInfo\n");
    return -1;  // TODO set errno
  }
  ...
  memset(&msg, 0, sizeof(struct shmipc_msg));
  ctx->ring_idx = shmipc_mgr_alloc_slot_dbg(service->shmipc_mgr);
  aprop_p = (struct allocatedPreadOpPacked *)IDX_TO_XREQ(service->shmipc_mgr,
                                                         ctx->ring_idx);
  prepare_allocatedPreadOp(&msg, aprop_p, ctx->fd, ctx->count, ctx->offset);
  aprop_p->alOp.shmid = shmid;
  aprop_p->alOp.dataPtrId = dataPtrId;
  ...
  shmipc_mgr_put_msg_nowait(service->shmipc_mgr, ctx->ring_idx, &msg);
  return 0;
}

// if submission is successful, return 0; otherwise, return -1
// the caller should allocate memory for ctx and free it after IO completion
int fs_allocated_pread_submit(struct async_ctx_rw *ctx, int fd, void *buf,
                              size_t count, off_t offset) {
  if (OpenLease::IsLocalFd(fd)) {
    int base_fd = OpenLease::FindBaseFd(fd);
    OpenLeaseMapEntry *entry = LeaseRef(base_fd);
  ...
  // pack args into ctx in case we may need them for resubmission
  ctx->fd = fd;
  ctx->buf = buf;
  ctx->count = count;
  ctx->offset = offset;
  return fs_allocated_pread_submit_internal(ctx);
}

// if I/O is done, put return code into *ret_code and return 0;
// otherwise, return -1
int fs_allocated_pread_poll(struct async_ctx_rw *ctx, ssize_t *ret_code) {
  struct shmipc_msg msg;
  FsService *service = (FsService *)ctx->service;
  int poll_rc = shmipc_mgr_poll_msg(service->shmipc_mgr, ctx->ring_idx, &msg);
  if (poll_rc == -1) return -1;

  struct allocatedPreadOpPacked *aprop_p =
      (struct allocatedPreadOpPacked *)IDX_TO_XREQ(service->shmipc_mgr,
                                                   ctx->ring_idx);
  struct allocatedPreadOp aprop;
  unpack_allocatedPreadOp(aprop_p, &aprop);
  ssize_t rc = aprop.rwOp.ret;

  shmipc_mgr_dealloc_slot(service->shmipc_mgr, ctx->ring_idx);

  if (rc < 0) {
    if (handle_inode_in_transfer(static_cast<int>(rc)) ||
        checkUpdateFdWid(static_cast<int>(rc), ctx->fd)) {
      // do retry
      int submit_rc = fs_allocated_pread_submit_internal(ctx);
      if (submit_rc) throw std::runtime_error("Unexpected submission failure");
      return -2;  // non-zero means I/O is not done
    }
  }
  *ret_code = rc;
  return 0;
...
ssize_t fs_allocated_pread_wait(struct async_ctx_rw *ctx) {
  ssize_t rc;
  while (fs_allocated_pread_poll(ctx, &rc))
    ;
  return rc;
}

int fs_allocated_pwrite_submit_internal(struct async_ctx_rw *ctx) {
  int wid = -1;
  FsService *service = getFsServiceForFD(ctx->fd, wid);
  ctx->service = service;
  ...
  // fill the shmName and dataPtr offset
  int err = 0;
  threadMemBuf->getBufOwnerInfo(ctx->buf, true, shmid, dataPtrId, err);
  if (err) {
    fprintf(stderr, "fs_allocated_pwrite_internal: Error in getBufOwnerInfo\n");
    return -1;  // TODO set errno
  }
  ...
  memset(&msg, 0, sizeof(struct shmipc_msg));
  ctx->ring_idx = shmipc_mgr_alloc_slot_dbg(service->shmipc_mgr);
  apwop_p = (struct allocatedPwriteOpPacked *)IDX_TO_XREQ(service->shmipc_mgr,
                                                          ctx->ring_idx);
  prepare_allocatedPwriteOp(&msg, apwop_p, ctx->fd, ctx->count, ctx->offset);
  apwop_p->alOp.shmid = shmid;
  apwop_p->alOp.dataPtrId = dataPtrId;
...
int fs_allocated_pwrite_submit(struct async_ctx_rw *ctx, int fd, void *buf,
                               size_t count, off_t offset) {
  // pack args into ctx in case we may need them for resubmission
  ctx->fd = fd;
  ctx->buf = buf;
  ctx->count = count;
  ctx->offset = offset;
  return fs_allocated_pwrite_submit_internal(ctx);
}

int fs_allocated_pwrite_poll(struct async_ctx_rw *ctx, ssize_t *ret_code) {
  struct shmipc_msg msg;
  FsService *service = (FsService *)ctx->service;
  int poll_rc = shmipc_mgr_poll_msg(service->shmipc_mgr, ctx->ring_idx, &msg);
  if (poll_rc == -1) return -1;
  ...
  struct allocatedPwriteOpPacked *apwop_p =
      (struct allocatedPwriteOpPacked *)IDX_TO_XREQ(service->shmipc_mgr,
                                                    ctx->ring_idx);
  struct allocatedPwriteOp apwop;
  unpack_allocatedPwriteOp(apwop_p, &apwop);
  ssize_t rc = apwop.rwOp.ret;
  ...
  if (rc < 0) {
    if (handle_inode_in_transfer(static_cast<int>(rc)) ||
        checkUpdateFdWid(static_cast<int>(rc), ctx->fd)) {
      // do retry
      int submit_rc = fs_allocated_pwrite_submit_internal(ctx);
      if (submit_rc) throw std::runtime_error("Unexpected submission failure");
      return -2;  // non-zero means I/O is not done
    }
  }
...
ssize_t fs_allocated_pwrite_wait(struct async_ctx_rw *ctx) {
  ssize_t rc;
  while (fs_allocated_pwrite_poll(ctx, &rc))
    ;
  return rc;
}

...
MemBlockMeta *FsLibLinearListsAllocator::doAllocate(
  ...
  if (idx < 0) {
    ...
    fprintf(stderr, "FsLibLinearListsAllocator::doAllocate(): size not "
                    "supported:%u\n",
            sizeBytes);
    throw std::bad_alloc();
  }
  ...
  MemBlockMeta *curMetaPtr = nullptr;
  {
    ...
    if (curFreeList->empty()) {
      ...
      fprintf(stderr, "FsLibLinearListsAllocator::doAllocate() no free block "
                      "available for size:%u, totalAllocatedBytes = %lu\n",
              sizeBytes, totalAllocatedBytes_);
      throw std::bad_alloc();
    }
...
void FsLibLinearCacheAllocator::dumpToStdout() {
  ...
  for (uint i = 0; i < memArr_->numBlocks; ++i) {
    ...
    if (meta->flags & (MEM_BLOCK_META_IN_USE_MASK)) {
      fprintf(
          stdout, " mapKey:%u dataPtrOffset:%ld mid:%u bSize:%u numPages:%u", i,
          meta->dataPtrOffset, meta->shmInnerId, meta->bSize, meta->numPages);
...
int FsLibMemMng::init(bool initShm) {
  ...
  for (int i = 0; i < kBlockSizeNum; i++) {
    ...
    fprintf(stdout, "FsLibMemMng:init() using shm %s\n", shmNameStr.c_str());
...
void *shmOpenInit(int &fd, std::string &shmNameStr, uint64_t sizeBytes,
  ...
  if (shmFd < 0) {
    if (errno != EEXIST) {
      ...
      if (shmFd < 0) {
        fprintf(stderr, "shm_open(O_RDWR, %s) error:%s\n", shmName,
                strerror(errno));
        err = 1;
        return nullptr;
      }

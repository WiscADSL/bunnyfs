...
FsProc::FsProc(int tNum, int appNum, const char *readyFname,
               const char *exitFname)
    : readyFname(readyFname),
...
void FsProc::workerReady(void) {
  ...
  int fd = ::open(readyFname.c_str(), O_WRONLY | O_CREAT, 0666);
  if (fd < 0) {
    SPDLOG_ERROR("failed to create ready file {}", readyFname);
    return;
  }

  int rc = ::close(fd);
  if (rc < 0) {
    SPDLOG_ERROR("failed to close ready file {}", readyFname);
    return;
  }
}
...
#ifdef DO_SCHED
void FsProc::startAllocator() {
  allocator = new sched::Allocator(this);
  for (int aid = 0; aid < numAppProc; ++aid) {
    auto &v = allocator->append_view(aid);
    for (int wid = 0; wid < numThreads; ++wid) {
      auto &t = workerList[wid]->GetApp(aid)->getTenant();
      v.append_tenant(&t);
      allocator->add_total_resrc(t.get_resrc());
    }
  }
  allocator_thread = new std::thread(sched::Allocator::run, allocator);
}
#endif
...
void FsProc::startWorkers(
    std::vector<int> &shmOffsetVec,
    std::vector<std::vector<std::tuple<int, int, double, double>>>
        &workerAppConfigs,
    std::vector<CurBlkDev *> &devVec, std::vector<int> &workerCoresVec) {
  ...
  messenger = new FsProcMessenger(numThreads, DEFAULT_MESSENGER_BUFSIZE);
//  loadMng->setMessenger(messenger);
  ...
  auto masterWorker = new FsProcWorkerMaster(
      FsProcWorker::kMasterWidConst + 0, devVec[0], shmOffsetVec[0],
      workerAppConfigs[0], &workerRunning[0], nullptr);
#else
  ...
  for (int i = 0; i < numThreads; i++) {
    if (i == 0) {
      ...
      peripheryWorker = new FsProcWorkerServant(
          FsProcWorker::kMasterWidConst + i, devVec[i], shmOffsetVec[i],
          workerAppConfigs[i], &workerRunning[i], nullptr);
#else
      ASSERT_STATS_TYPE_NOT_VALID
#endif
...
//                          (curWorker->stats_recorder_.GetStatsCombPtr()));
  ...
  static_assert(std::atomic_bool::is_always_lock_free,
                "std::atomic_bool is NOT lock-free!");
  ...
  // init monitor
  //  loadMngThread_ =
  //      new std::thread(&worker_stats::LoadMngType::loadManagerRunner,
  //      loadMng);
...
FsProcWorker *FsProc::startInlineWorker(CurBlkDev *dev, bool loadFs) {
  std::vector<std::tuple<int, int, double, double>> appConfigs;  // empty config
#if defined SCALE_USE_BUCKETED_DATA
  auto *curWorker = reinterpret_cast<FsProcWorker *>(new FsProcWorkerMaster(
      FsProcWorker::kMasterWidConst, dev, 0, appConfigs, &workerRunning[0],
      loadMng->getWorkerStatsPtr(FsProcWorker::kMasterWidConst)));
#elif defined SCALE_USE_ATOMIC_DATA
  auto *curWorker = reinterpret_cast<FsProcWorker *>(
      new FsProcWorkerMaster(FsProcWorker::kMasterWidConst, dev, 0, appConfigs,
                             &workerRunning[0], nullptr));
#else
  ASSERT_STATS_TYPE_NOT_VALID
#endif
...
void FsProc::CheckNewedInMemInodeDst(int wid, cfs_ino_t ino, int pid, int tid) {
  if (wid != FsProcWorker::kMasterWidConst) return;
  auto master = reinterpret_cast<FsProcWorkerMaster *>(workerList[wid]);
  int dst_wid = master->GetPidtidHandlingWid(pid, tid);
  if (dst_wid <= 0) return;

  // Ignore if ino is already in the list,
  // which may happen when the same file is opened by multiple processes/threads
  for (auto &e : master->pending_newed_inodes) {
    if (e.ino == ino) return;
  }

  master->pending_newed_inodes.push_back({
      .ino = ino,
      .pid = pid,
      .tid = tid,
      .dst_wid = dst_wid,
...
// TODO: init tenant; currently only use default parameters
AppProc::AppProc(FsProcWorker *worker, int aid, int shmBaseOffset,
                 AppCredential &cred, uint32_t cache_size, int64_t bandwidth,
                 int64_t cpu_cycles)
    : worker(worker),
      aid(aid),
      cred(cred),
      fdIncr(worker->getWid() * 10000000 + kFdBase)
#ifdef DO_SCHED
      ,
      tenant(worker->getWid(), aid, this, cache_size, bandwidth, cpu_cycles)
#endif
{
  initShm(aid, shmBaseOffset);
  SPDLOG_INFO(
      "Worker-{}: Create app [aid={}, shmoff={}] with cache={}MB, "
      "bw={}MB/s, cpu={}",
      worker->getWid(), aid, shmBaseOffset,
      sched::params::blocks_to_mb(cache_size),
      sched::params::blocks_to_mb(bandwidth),
      double(cpu_cycles) / sched::params::worker_avail_cycles_per_second);
}

void AppProc::initShm(int aid, int shmBaseOffset) {
  int shmKey = FS_SHM_KEY_BASE + shmBaseOffset + aid;
  char shmfile[128];
  // TODO have a constant for this format?
  snprintf(shmfile, 128, "/shmipc_mgr_%08d", shmKey);

  // We create the memory on server side, hence 1.
  shmipc_mgr = shmipc_mgr_init(shmfile, RING_SIZE, 1);
  if (shmipc_mgr == NULL) {
    SPDLOG_ERROR(
        "Failed to initialize shared memory for AppProc aid={}, shm_key={}",
        aid, shmKey);
...
void FsReqIoUnitHelper::submittedIoUnitReqDone(cfs_bno_t iouNo) {
  SPDLOG_DEBUG("submittedIoUnitReqDone iouNo:{}", iouNo);
  ...
  auto item = bReq->getBufferItem();
  item->setInMem();
  auto pool = item->getPool();
  if (pool != nullptr) {
    pool->releaseBlock(item);
  }
...
void FsReq::initReqFromCop(AppProc *curApp, off_t curSlotId,
  ...
  // // The line below has incorrect/unimplemented formatting; disable
...
/* No longer use pending map */
/*******************************************************************************
void FsReq::addBlockSubmitReq(uint32_t blockNo, BlockBufferHandle item,
...
void FsReq::addSectorSubmitReq(uint32_t sectorNo, BlockBufferHandle bufItem,
                               char *dst, FsBlockReqType type) {
...
// void FsReq::addBlockWriteReq(uint32_t blockNo, BlockBufferHandle bufItem,
//                             char *src) {
...
/* No longer use pending map */
/*******************************************************************************
void FsReq::addSectorWriteReq(uint32_t sectorNo, BlockBufferHandle bufItem,
                              char *src) {
  // TODO (jingliu)
...
bool FsReq::writeReqDone(uint64_t blockNo, uint64_t blockNoSeqNo,
  ...
  //      BlockBufferHandle item = bReq->getBufferItem();
...
BlockReqPool::BlockReqPool()
    : pool_(new BlockReq[kPerWorkerBlockReqPoolCapacity]()) {
  block_req_queue_.reserve(kPerWorkerBlockReqPoolCapacity);
  for (int i = 0; i < kPerWorkerBlockReqPoolCapacity; ++i)
    block_req_queue_.emplace_back(&pool_[i]);
}

BlockReqPool::~BlockReqPool() { delete[] pool_; }

// caller must call init()!
BlockReq *BlockReqPool::alloc() {
  if (block_req_queue_.empty())
    throw std::runtime_error("No BlockReq available!");
  auto r = block_req_queue_.back();
  block_req_queue_.pop_back();
  return r;
}

void BlockReqPool::free(BlockReq *r) {
  r->clear();
  block_req_queue_.emplace_back(r);
}
...
void BlockReq::init(uint64_t blockNo, BlockBufferHandle bufItem, char *dst,
                    FsBlockReqType t) {
  this->blockNo = blockNo;
  this->blockNoSeqNo = 0;
  this->isSubmitted = false;
  this->memPtr = dst;
  this->bufferItem = bufItem;
  this->reqType = t;
  this->submitted_ts = 0;
  this->primary_fs_req = nullptr;
  this->secondary_fs_reqs = nullptr;
}
...
BlockBufferHandle BlockReq::getBufferItem() { return bufferItem; }

int BufferFlushReq::initFlushReqs(int index) {
  bool canFlush = true;
  if (index > 0) {
    srcBuf->flusher.doFlushByIndex(index, canFlush, toSubmitFlushBlocks);
  } else {
    srcBuf->flusher.doFlush(canFlush, toSubmitFlushBlocks);
  }
  assert(canFlush);
  for (auto handle : toSubmitFlushBlocks) {
    block_no_t bno = handle.get_key();
    SPDLOG_DEBUG("initFlushReqs bno:{} idx:{}", bno, handle->getIndex());
    if (!handle->isDirty()) {
      if (fsReq != nullptr) {
        SPDLOG_WARN("initFlushReqs bno:{} not_dirty reqType:{} ino:{}", bno,
                    getFsReqTypeOutputString(fsReq->getType()),
                    fsReq->getTargetInode()->inodeData->i_no);
      } else {
        SPDLOG_WARN("initFlushReqs bno:{} not_dirty no req", bno);
      }
      throw std::runtime_error("buffer is not dirty");
    }
    auto req = new BlockReq(bno, handle, handle->getBufPtr(),
...
int BufferFlushReq::submitFlushReqs() {
  ...
  while (!toSubmitFlushBlocks.empty()) {
    ...
    bno = toSubmitFlushBlocks.front().get_key();
    BlockReq *block_req = flushBlockReqMap[bno];
#ifndef DO_SCHED
    // directly submit to the device
    rc = submitWorker->submitAsyncBufFlushWriteDevReq(this, block_req);
    if (rc < 0) {  // submit fail
      SPDLOG_INFO("submitFlushReq submit fail. bno:{}", bno);
      break;
    } else {
      numSubmit++;
...
#else
    // add to the tenant's block request queue
    block_req->set_buf_flush_req(this);
    fsReq->get_tenant()->add_blk_queue(block_req, fsReq);
    numSubmit++;
    toSubmitFlushBlocks.pop_front();
    submittedFlushBlocks.insert(bno);
#endif
...
void BufferFlushReq::blockReqFinished(block_no_t blockNo) {
  ...
  if (!toSubmitFlushBlocks.empty()) {
    block_no_t bno = toSubmitFlushBlocks.front().get_key();
    BlockReq *block_req = flushBlockReqMap[bno];
#ifndef DO_SCHED
    // directly submit to the device
    int rc = submitWorker->submitAsyncBufFlushWriteDevReq(this, block_req);
    assert(rc >= 0);
#else
    // add to the tenant's block request queue
    block_req->set_buf_flush_req(this);
...
void BufferFlushReq::flushDonePropagate() {
  for (auto [bno, req] : flushBlockReqMap) {
    srcBuf->unsetBlockDirty(req->getBufferItem());
    delete req;
  }
  flushBlockReqMap.clear();
  int rc = srcBuf->flusher.doFlushDone();
  if (rc < 0) {
    if (fsReq != nullptr) {
      SPDLOG_ERROR("flushDonePropagate reqType:{} ino:{}",
                   getFsReqTypeOutputString(fsReq->getType()),
                   fsReq->getTargetInode()->inodeData->i_no);
    } else {
      SPDLOG_ERROR("flushDonePropagate no req");
    }
  ...
  if (isAssociatedWithFsReq()) {
    srcBuf->flusher.addFgFlushInflightNum(-1);
    if (enableTrace_) {
      doneTs = tapFlushTs();
      fprintf(stderr,
              "--JL-- flushUs:%lu num_blk:%lu submit_ts:%lu "
              "done_ts:%lu inum:%u\n",
              PlatformLab::PerfUtils::Cycles::toMicroseconds(doneTs - submitTs),
    ...
    srcBuf->flusher.removeFgFlushWaitIndex(fsReq->getFileInum());
...
void *AppProc::getDataPtrByShmIdAndDataId(uint8_t shmId,
  ...
  if (it == shmIdArrMap.end()) {
    ...
    if (memArrPtr == nullptr) {
      throw std::runtime_error("bad shmid received from client. appIdx: " +
...
std::pair<uint8_t, void *> AppProc::initNewAllocatedShm(
    ...
    fslib_malloc_block_cnt_t shmNumBlocks) {
  SPDLOG_INFO("initNewAllocatedShm: {}", shmName);
...
int AppProc::invalidateAppShm() {
  std::string shmNames;
  for (auto ele : shmNameMemArrMap) {
    shmNames += ele.first + " ";
  }
  SPDLOG_INFO("invalidateAppShm: {}", shmNames);
...
struct clientOp *MockAppProc::emulateGetClientOp(int sid) { return nullptr; }

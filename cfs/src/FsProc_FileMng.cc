...
int FileMng::processReq(FsReq *req) {
  ...
  switch (req->getType()) {
...
#if defined(DO_SCHED) && defined(_EXTENT_FOR_LDB_)
      req->getClientOp()->op.unlink.ret = 0;
      fsWorker_->submitFsReqCompletion(req);
      SPDLOG_WARN("unlink({}) is not supported in sched mode; will ignore",
                  fmt::join(req->pathTokens, "/"));
      break;
#endif
    ...
    case FsReqType::RENAME:
      ...
      fsWorker_->submitFsReqCompletion(req);
      SPDLOG_WARN("rename({}, {}) is not supported in sched mode; will ignore",
                  fmt::join(req->pathTokens, "/"),
                  fmt::join(req->dstPathTokens, "/"));
      break;
#endif
...
bool FileMng::exportInode(cfs_ino_t inum, ExportedInode &exp) {
  ...
  if (fsWorker_->NumInProgressRequests(inum) != 0) {
    SPDLOG_WARN("Cannot export inode {} as there are pending requests", inum);
    return false;
  }
  ...
  if (exp.inode == nullptr) {
    SPDLOG_WARN("Cannot export inode {} as it is not in memory", inum);
    return false;
  }
  // assert(exp.inode);
...
void FileMng::processPwrite(FsReq *req) {
  if (req->getState() == FsReqState::PWRITE_MODIFY) {
    ...
    SPDLOG_DEBUG("processPwrite offset:{} count:{}", fobjStartOff, reqCount);
    if (reqCount > RING_DATA_ITEM_SIZE || reqCount <= 0) {
...
void FileMng::processRename(FsReq *req) {
  ...
  if (req->getState() == FsReqState::RENAME_TRANSACTION) {
    ...
    if (req->isDstExist()) {
      ...
      [[maybe_unused]] int rt = fsImpl_->dentryPointToNewInode(

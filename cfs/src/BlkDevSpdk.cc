...
int BlkDevSpdk::submitDevReq(uint64_t blockNo, uint64_t blockNoSeqNo,
                             ...
                             uint32_t curBlockSize, void *ctx_payload) {
  ...
  ctx_ptr->buf = data;
  ctx_ptr->ctx_payload = ctx_payload;
...
int BlkDevSpdk::read(uint64_t blockNo, char *data, void *ctx_payload) {
  return submitDevReq(blockNo, /*blockNoSeqNo*/ 0, data,
                      BlkDevReqType::BLK_DEV_REQ_READ, /*curBlockSize*/ 0,
                      ctx_payload);
}
...
int BlkDevSpdk::initWorker(int wid) {
  ...
  if (tidQpairList[tid] != nullptr) {
    SPDLOG_DEBUG("initWorker: this worker has been set to SpdkDev, but it's ok");
    return -1;
  }
  ...
  std::lock_guard<std::mutex> lock(lmtx);
  {
    ...
    lastReportTs[curWid] = PlatformLab::PerfUtils::Cycles::rdtsc();
    if (tid != curWid) {
  ...
  SPDLOG_DEBUG("initWorker Completed for tid:{} wid:{}", tid, wid);
  return 0;
}
...
void BlkDevSpdk::statsReport() {
  ...
  uint64_t curTs = PlatformLab::PerfUtils::Cycles::rdtsc();
  auto curAllocCntPtr = &(threadAllocCntVec[curWid]);
  float time_lapse =
      PlatformLab::PerfUtils::Cycles::toSeconds(curTs - lastReportTs[curWid]);
...
struct BdevIoContext *BlkDevSpdk::allocReqContext(cfs_tid_t tid) {
  ...
  // do the allocation
  bdev_reqid_t rid = tidUnusedReqidList[tid].back();
  tidUnusedReqidList[tid].pop_back();
...
int BlkDevPosix::read(uint64_t blockNo, char *data, void *ctx_payload) {
...
int BlkDevPosix::readSector(uint64_t secNo, char *data, void* ctx_payload) {

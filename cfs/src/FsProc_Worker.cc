...
FsProcWorker::FsProcWorker(
    int wid, CurBlkDev *d, int shmBaseOffset,
    std::vector<std::tuple<int, int, double, double>> &appConfigs,
...
void FsProcWorker::ProcessLmJoinAll(LmMsgJoinAllCtx *ctx) {
  throw std::runtime_error("This load management is no longer supported!");
  /* We have remove `inodeInProgressFsReqMap_` due to its heavy overhead.
   * For now, we simply don't support this operation that need info of from
...
void FsProcWorker::schedMigrateInode(
    AppProc *app, const std::vector<std::tuple<int, int>> &inode_move) {
  std::vector<cfs_ino_t> migratable_inodes;
  for (auto ino : app->GetInos()) {
    InMemInode *inode = fileManager->GetInMemInode(ino);
    if (inode == nullptr) {
      SCHED_LOG_NOTICE(
          "Cannot export inode %d for App-%d, because it is not in memory", ino,
          app->getAid());
      continue;
    }
    if (NumInProgressRequests(ino) > 0) {
      SCHED_LOG_NOTICE(
          "Cannot export inode %d for App-%d, because it has pending "
          "requests",
          ino, app->getAid());
      continue;
    }
    if (inode->isDeleted) {
      SCHED_LOG_NOTICE("Inode %d is deleted, continue to the next one", ino);
      continue;
    }
    if (inode->isLocked) {
      SCHED_LOG_NOTICE("Inode %d is locked, continue to the next one", ino);
      continue;
    }
    if (inode->inodeData->type == T_DIR) {
      throw std::runtime_error("Directory should not be in `accessed_ino`");
    }
    migratable_inodes.emplace_back(ino);
  }
  std::shuffle(migratable_inodes.begin(), migratable_inodes.end(),
               std::default_random_engine(std::random_device()()));

  auto pid = app->getPid();
  auto aid = app->getAid();
  auto inode_it = migratable_inodes.begin();

  for (const auto &[dst_wid, num_inodes] : inode_move) {
    if (num_inodes == 0) continue;
    SCHED_LOG_NOTICE("App-%d: Moving %d inodes from Worker-%d to Worker-%d",
                     aid, num_inodes, wid, dst_wid);
    int cnt = 0;
    while (inode_it != migratable_inodes.end()) {
      cfs_ino_t ino = *inode_it;
      SCHED_LOG_DEBUG(
          "App-%d: Moving inode %u from Worker-%d to "
          "Worker-%d",
          aid, ino, wid, dst_wid);

      auto old_owner_callback =
          [](FileMng *mng, const FileMng::ReassignmentOp::Ctx *reassign_ctx,
             void *myctx) { SPDLOG_DEBUG("ProcessPendingNewed old_owner"); };
      struct NewOwnerCtx {
        pid_t pid;
        std::vector<int> tids;
        cfs_ino_t ino;
      ...
      auto new_owner_callback =
          [](FileMng *mng, const FileMng::ReassignmentOp::Ctx *reassign_ctx,
            ...
            for (auto tid : newctx->tids) {
              app->AccessIno(tid, newctx->ino);
            }
            delete newctx;
          };
      FileMng::ReassignmentOp::OwnerExportThroughPrimary(
          fileManager, ino, dst_wid, old_owner_callback, nullptr,
          new_owner_callback, new_ctx);

      app->EraseIno(ino);
      ++cnt;
      inode_it++;
      if (cnt >= num_inodes) break;
    }
    if (cnt < num_inodes) {
      // this is a big issue... we want to know it from cmdline log too
      SCHED_LOG_WARNING(
          "App-%d: Not enough inodes in Worker-%d, migrated %d, to migrate %d",
          aid, wid, cnt, num_inodes);
      SPDLOG_WARN(
          "App-{}: Not enough inodes in Worker-{}, migrated {}, to migrate {}",
          aid, wid, cnt, num_inodes);
      // our new impl should prevent this situation
      throw std::runtime_error("Fail to migrate inodes as expected");
      return;
    }
  }
}

void FsProcWorker::ProcessSchedNewResrcAlloc(sched::AllocDecision *decision) {
#ifdef DO_SCHED
  int aid = decision->aid;
  assert(appMap.contains(aid));
  const auto &app = appMap[aid];
  auto &t = app->getTenant();
  SCHED_LOG_NOTICE("Worker-%d: Update resources for App-%d", getWid(),
                   decision->aid);
  t.set_resrc(decision->resrc);
  if (sched::params::policy::cache_partition)
    fileManager->fsImpl_->adjustCacheSize(sched::Tag{.tenant = &t});
  int num_inodes_to_migrate = 0;
  for (const auto &[dst_wid, nfiles] : decision->inode_move)
    num_inodes_to_migrate += nfiles;
  if (num_inodes_to_migrate > 0) {
    // NOTE: the migration support is deprecated
    SPDLOG_WARN(
        "FsProcWorker::ProcessSchedNewResrcAlloc: inode migration is "
        "deprecated");
    t.set_drain_for_migration(std::move(decision->inode_move));
    // if we can do it now, do it.
    if (t.should_migrate()) {
      schedMigrateInode(app, t.get_pending_inode_move());
...
int FsProcWorker::CheckFutureRoutingOnReqCompletion(AppProc *app, int tid,
  ...
  if (dst_wid >= 0 && dst_wid != getWid()) {
    ...
    //         PlatformLab::PerfUtils::Cycles::toNanoseconds(PlatformLab::PerfUtils::Cycles::rdtsc()),
...
// initialize all apps with given credentials (could be empty)
void FsProcWorker::InitApps() {
  for (auto [aid, cache_mb, bw_mb, cpu_ratio] : appConfigs) {
    AppCredential cred(aid, 9, 9);
    assert(!appMap.contains(aid));
    auto app = new AppProc(
        this, aid, shmBaseOffset, cred, sched::params::mb_to_blocks(cache_mb),
        sched::params::mb_to_blocks(bw_mb),
        sched::params::worker_avail_cycles_per_second * cpu_ratio);
    appMap.emplace(app->getPid(), app);
    appList.emplace_back(app);
  }
  for (int aid = 0; aid < gFsProcPtr->getNumApps(); ++aid) {
    if (appMap.contains(aid)) continue;
    AppCredential cred(aid, 9, 9);
    auto app = new AppProc(this, aid, shmBaseOffset, cred, 0, 0, 0);
    appMap.emplace(app->getPid(), app);
    appList.emplace_back(app);
  }
}
...
int FsProcWorker::submitDevAsyncReadReqCompletion(struct BdevIoContext *ctx) {
  ...
  BlockReq *block_req = (BlockReq *)ctx->ctx_payload;
  assert(block_req);

  /*
    the code below is disabled because we find maintaining the pending reqs
    is too costly; this is not useful if there is no two fsReq accesses the
    same block concurrently. we instead let BlockReq keeps reverses pointers
    to FsReq for this purposes. we hope this saves the cost of so many
    unnecessary memory manage overhead of using unordered_map etc.
  */
  /*****************************************************************************
  ...
  *****************************************************************************/
  uint64_t block_io_latency = block_req->get_latency();

  auto primary_fs_req = block_req->get_primary_fs_req();
  auto secondary_fs_reqs = block_req->get_secondary_fs_reqs();
  assert(primary_fs_req);

  // we only count it towards the primary tenant for now, because it is the
  // tenant that submits this IO
#ifdef DO_SCHED
  primary_fs_req->get_tenant()->add_latency(block_io_latency);
#endif

  primary_fs_req->dec_pending();
  if (primary_fs_req->numTotalPendingIoReq() == 0)
    submitReadyReq(primary_fs_req);
  if (secondary_fs_reqs) {
    for (auto r : *secondary_fs_reqs) {
      r->dec_pending();
      if (r->numTotalPendingIoReq() == 0) submitReadyReq(r);
    }
  }

  // set buffer item as in-memory (no need to use multiple maps for this...)
  auto item = block_req->getBufferItem();
  item->set_IO_done();
  auto pool = item->getPool();
  if (pool) pool->releaseBlock(item);
  block_req_pool.free(block_req);
...
int FsProcWorker::submitBlkWriteReqCompletion(struct BdevIoContext *ctx) {
  ...
  if (flushReqIt != blockFlushReqMap.end()) {
    BufferFlushReq *flushReq = flushReqIt->second;
    ...
    if (flushReq->checkValidFlushReq(ctx->blockNo, ctx->blockNoSeqNo)) {
      flushReq->fsReq->startOnCpuTimer();
      submitBlkFlushWriteReqCompletion(ctx->blockNo, flushReq);
      flushReq->fsReq->stopOnCpuTimer();
    }
  } else {
    uint64_t blockNo = ctx->blockNo;
...
int FsProcWorker::submitAsyncReadDevReq(FsReq *fsReq, BlockReq *blockReq) {
  SPDLOG_DEBUG("submitAsyncReadDevReq: blockNo:{}", blockReq->getBlockNo());
  // it seems the original implementation does not support two fs req waiting
  // for the same block req
  assert(fsReq == blockReq->get_primary_fs_req());
  // primary_fs_req is equivalent to curUnitPendingSubmitFsReq[blockNo]
  // secondary_fs_req is equivalent to other in curUnitPendingFsReqs[blockNo]
...
#ifndef DO_SCHED
  // just submit to device...
  return doSubmitAsyncRead(fsReq, blockReq);
#else
  AppProc *app = fsReq->getApp();
  if (app != nullptr) {  // fsReq created from genGenericRequest has no app
    app->getTenant().add_blk_queue(blockReq, fsReq);
  } else {
    SPDLOG_ERROR("FsReq has no app");
    throw std::runtime_error("FsReq has no app");
  }
  return 0;
#endif
}

int FsProcWorker::doSubmitAsyncRead(FsReq *fsReq, BlockReq *blockReq) {
  // Do actual submission to device
  int rc;
  if (blockReq->getReqType() == FsBlockReqType::READ_NOBLOCKING) {
    rc = dev->read(blockReq->getBlockNo(), blockReq->getBufPtr(),
                   /*ctx_payload*/ blockReq);
...
int FsProcWorker::submitAsyncWriteDevReq(FsReq *fsReq, BlockReq *blockReq) {
  ...
  if (!dev->isSpdk()) {
    rc = dev->write(blockReq->getBlockNo(), blockReq->getBlockNoSeqNo(),
                    blockReq->getBufPtr());
    if (rc > 0) {
      // TODO: call dataBlockBuf_.setItemDirty(false) here
  ...
  uint64_t cur_req_seq = PlatformLab::PerfUtils::Cycles::rdtsc();
...
int FsProcWorker::submitAsyncBufFlushWriteDevReq(BufferFlushReq *bufFlushReq,
  ...
  int rc;
  uint64_t curReqSeq = PlatformLab::PerfUtils::Cycles::rdtsc();
...
int FsProcWorker::submitFsReqCompletion(FsReq *fsReq) {
  ...
  assert(app != nullptr);
#ifdef DO_SCHED
  app->getTenant().record_req_done();  // --num_reqs_inflight
#endif
...
void FsProcWorker::recordInProgressFsReq(FsReq *req, InMemInode *inodePtr) {
  ...
  if (inodePtr->i_no != inodePtr->inodeData->i_no) {
    SPDLOG_ERROR(
        ...
        "inodePtr->inodeData:{}",
        inodePtr->i_no, inodePtr->inodeData->i_no, (void *)inodePtr->inodeData);
    assert(false);
  }

  if (inodePtr->inodeData->type != T_FILE) return;

  auto prev_inode = req->get_inode_in_prog();
  if (!prev_inode) {
    inodePtr->num_fs_req_in_prog++;
    req->set_inode_in_prog(inodePtr);
  } else {
    assert(prev_inode == inodePtr);
  }
}

void FsProcWorker::delInProgressFsReq(FsReq *fsReq) {
  assert(fsReq != nullptr);
  InMemInode *inodePtr = fsReq->get_inode_in_prog();
  assert(inodePtr);
  assert(inodePtr == fileManager->GetInMemInode(fsReq->getFileInum()));
  assert(inodePtr->num_fs_req_in_prog > 0);
  inodePtr->num_fs_req_in_prog--;
  fsReq->set_inode_in_prog(nullptr);
}
...
// NOTE: this function is disabled for now
//       we don't support adding app at runtime
//       one must pre-configure each app's resource share from cmdline
  ...
  if (appmap_fill_loop_counter_ == kAppMapFillLoopCnt || init_app_num == 0) {
    ...
    if (!result_vec.empty()) {
      ...
      for (auto [cur_pid, cur_cred] : result_vec) {
        ...
        // if (cur_app_it == appMap.end()) InitCredToEmptyApp(*cur_cred);
        delete cur_cred;
      }
    }
...
int FsProcWorker::pollReqFromApps() {
  ...
  for (auto app : appList) {
    do {
#ifdef DO_SCHED
      uint64_t begin_ts = PlatformLab::PerfUtils::Cycles::rdtsc();
#endif
      ...
      auto &t = app->getTenant();
      reqPtr->set_tenant(&t);
      t.add_recv_queue(reqPtr);
      uint64_t end_ts = PlatformLab::PerfUtils::Cycles::rdtsc();
      t.record_cpu_consump(end_ts - begin_ts);
#endif
    } while (true);
  }
...
#else
bool FsProcWorker::processRecvReadyQueue(sched::Tenant &t) {
  FsReq *req = t.pop_recv_queue();
  if (!req) return false;
  processReqOnRecv(req);  // which will call startOnCpuTimer
  return true;
}

bool FsProcWorker::processInternalReadyQueue(sched::Tenant &t) {
  FsReq *req = t.pop_intl_queue();
  if (!req) return false;
  req->startOnCpuTimer();
...
uint64_t FsProcWorker::processBlockReadyQueue(sched::Tenant &t) {
  uint64_t num_blks_submitted = 0;
  int rc;
  while (true) {
    FsReq *fs_req;
    BlockReq *blk_req = t.pop_blk_queue(fs_req);

    if (!blk_req) return num_blks_submitted;
    switch (blk_req->getReqType()) {
      case FsBlockReqType::READ_NOBLOCKING:
      case FsBlockReqType::READ_NOBLOCKING_SECTOR:
        rc = doSubmitAsyncRead(fs_req, blk_req);
        if (rc >= 0) {
          num_blks_submitted += 1;
        } else {
          SPDLOG_WARN("Device async read submit fail with bno={}",
                      blk_req->getBlockNo());
        }
        break;
      case FsBlockReqType::WRITE_NOBLOCKING:
      case FsBlockReqType::WRITE_NOBLOCKING_SECTOR:
        fs_req->startOnCpuTimer();
        rc = submitAsyncBufFlushWriteDevReq(blk_req->get_buf_flush_req(),
                                            blk_req);
        fs_req->stopOnCpuTimer();
        if (rc >= 0) {
          num_blks_submitted += 1;
        } else {
          SPDLOG_WARN("Device async write submit fail with bno={}",
                      blk_req->getBlockNo());
        }
        break;
      default:
        throw std::runtime_error("unknown block req type");
    }
  }
}
#endif
...
void FsProcWorker::primaryHandleUnknownFdReq(FsReq *req) {
  ...
  if (ino == 0) {
    SPDLOG_INFO("POSIX_EBADF getWid:{} pid:{} fd:{} reqType:{}", getWid(),
                req->getPid(), req->fd, req->getType());
    // fd not found
...
#ifndef DO_SCHED
bool FsProcWorker::workerRunLoopInner() {
  bool loopEffective = false;

  // NOTE: disable adding new app after initialization
  //       for now we only pre-configure each app's share from cmdline
  // RefillAppMap();
...
bool FsProcWorker::workerRunLoopInner() {
  ...
  // NOTE: disable adding apps at runtime for now
  // RefillAppMap();
  int numAppReqPolled = pollReqFromApps();
  loopEffective |= (numAppReqPolled > 0);
  ...
  uint64_t now_ts = PlatformLab::PerfUtils::Cycles::rdtsc();
  uint64_t elapsed = now_ts - cpu_prog_epoch_ts;  // within an epoch
  if (elapsed > sched::params::cycles_per_cpu_epoch) {
    for (auto app : appList) app->getTenant().reset_cpu_prog();
    cpu_prog_epoch_ts = now_ts;
  }

  for (int i = 0; i < sched::params::num_reqs_per_loop; ++i) {
    bool has_work_done = false;
    elapsed = PlatformLab::PerfUtils::Cycles::rdtsc() - cpu_prog_epoch_ts;
    // we expect the number of apps are not crazily large, so just do sorting is
    // not too much slower than using a heap, but much simpler
    std::sort(appList.begin(), appList.end(), [](AppProc *&lhs, AppProc *&rhs) {
      return lhs->getTenant().get_cpu_prog() < rhs->getTenant().get_cpu_prog();
    });
    for (auto app : appList) {  // ordered by cpu progress
      auto &t = app->getTenant();
      if (!t.can_sched(elapsed)) continue;
      has_work_done |= processRecvReadyQueue(t);
      has_work_done |= processInternalReadyQueue(t);
      assert(has_work_done);
      break;
    }
    loopEffective |= has_work_done;
    if (!has_work_done) break;  // all queues are empty, no point to proceed
  }

  // submit block requests that are ready (if any)
  // also check if migration is necessary
  for (auto app : appList) {
    auto &t = app->getTenant();
    if (t.should_migrate()) {
      schedMigrateInode(app, t.get_pending_inode_move());
    ...
    loopEffective |= processBlockReadyQueue(t) > 0;
  }

  /* For DO_SCHED, we disable background flush; all flush must be triggered by
   * explict fsync/fdatasync and accounted for bandwidth consumption */
  /*****************************************************************************
  auto numFlushBlock = fileManager->checkAndFlushBufferDirtyItems();
  loopEffective |= (numFlushBlock > 0);
  *****************************************************************************/
...
void FsProcWorker::processFsProcMessage(const FsProcMessage &msg) {
  ...
  switch (msg_type) {
    ...
    case FsProcMessageType::kSCHED_NewResrcAlloc: {
      sched::AllocDecision *ctx = static_cast<sched::AllocDecision *>(msg.ctx);
      ProcessSchedNewResrcAlloc(ctx);
      delete ctx;
    } break;
    default:
...
void FsProcWorker::opStatsAccountSingleOpDone(FsReqType reqType, size_t bytes) {
  ...
  if (opStatsPtr_->numOpDone == 0) {
    opStatsPtr_->firstOpNano = PlatformLab::PerfUtils::Cycles::toNanoseconds(
        PlatformLab::PerfUtils::Cycles::rdtscp());
  }
  opStatsPtr_->lastOpNano = PlatformLab::PerfUtils::Cycles::toNanoseconds(
      PlatformLab::PerfUtils::Cycles::rdtscp());
  opStatsPtr_->numOpDone++;
  opStatsPtr_->doneBytes += bytes;
}

FsProcWorkerMaster::FsProcWorkerMaster(
    int w, CurBlkDev *d, int shmBaseOffset,
    std::vector<std::tuple<int, int, double, double>> &appConfigs,
    std::atomic_bool *workerRunning, PerWorkerLoadStatsSR *stats)
    : FsProcWorker(w, d, shmBaseOffset, appConfigs, workerRunning, stats) {
...
void FsProcWorkerMaster::workerRunLoop() {
  SCHED_LOG_NOTICE("Worker 0 (master) starts to run");
  ...
  // install App credentials. This needs to be done before
  // initInMemDataAfterDevReady, which initializes the BlockBuffer
  InitApps();
  ...
  sleep(1);
  // FIXME: I think it is a terrible idea to use sleep... It may work, but it's
  // ugly... Please fix it in the future...

#ifdef DO_SCHED
  // start allocator
  gFsProcPtr->startAllocator();
#endif
  ...
  while (*workerRunning) {
    ...
    if (!loopEffective) idle_stat.stop();
    /* We disable stats_recorder here because we are not using it now */
    /***************************************************************************
    ...
    ***************************************************************************/
    ...
    /* We disable checking exit file. Just use SIGINT to terminate */
    /***************************************************************************
    // master needs to check the EXIT event (a file's appearance)
    if ((loopCountBeforeCheckExit++) > FsProc::kCheckExitLoopNum) {
      bool exit = checkFileExistance(gFsProcPtr->exitSignalFileName.c_str());
...
#ifdef DO_SCHED
  for (auto app : appList) SPDLOG_INFO("{}", app->getTenant().to_string());
#endif
  ...
  if (op_ptr->dst_wid == FsProcWorker::kMasterWidConst) {
    ...
    if (op_ptr->src_wid == FsProcWorker::kMasterWidConst) {
      ...
      if (req->getClientOp()->op.threadReassign.flag != FS_REASSIGN_FUTURE) {
        auto ino_set = req->getApp()->GetAccessedInoAndErase(tid);
  ...
  if (dst_wid == FsProcWorker::kMasterWidConst) {
    ...
    if (src_wid == FsProcWorker::kMasterWidConst) {
      ...
      if (ctx->time_range != TimeRange::kFuture) {
        auto ino_set = app->GetAccessedInoAndErase(tid);
        // from master to others
        SPDLOG_DEBUG(
...
bool FsProcWorkerMaster::submitFileInodeSplitMsg(int dstWid,
  ...
  logger->debug("[splitting] # of inProgres reqs:{}",
                inodePtr->num_fs_req_in_prog);
...
int FsProcWorkerMaster::ProcessPendingNewedInodesMigration() {
  ...
  using FR = FileMng::ReassignmentOp;
  int num_sent = 0;
  auto it = pending_newed_inodes.begin();
  while (it != pending_newed_inodes.end()) {
    auto app = GetApp(it->pid);
    cfs_ino_t cur_ino = it->ino;
    assert(it->dst_wid > 0);
    ...
    SPDLOG_DEBUG("ProcessPendingNewed dst_wid:{} ino:{}", it->dst_wid, cur_ino);
    ...
    auto new_owner_callback = [](FileMng *mng, const FR::Ctx *reassign_ctx,
      ...
      auto app = mng->fsWorker_->GetApp(newctx->pid);
      if (mng->GetInMemInode(newctx->ino)->inodeData->type == T_FILE) {
        app->AccessIno(newctx->tid, newctx->ino);
      }
      delete newctx;
    };
    FR::OwnerExportThroughPrimary(fileManager, cur_ino, it->dst_wid,
                                  old_owner_callback, nullptr,
                                  new_owner_callback, new_ctx);

    app->EraseIno(it->tid, cur_ino);
    num_sent++;
    it = pending_newed_inodes.erase(it);
  }
  return num_sent;
}
...
void FsProcWorkerMaster::recvFileInodeJoinMsg(
  ...
  if (dstWid != FsProcWorker::kMasterWidConst) {
    ...
    fileManager->installDataBlockBufferSlot(inode, msg->dataBlocks);
...
void FsProcWorkerMaster::addRecentlyReassignedInodeFds(ExportedInode &exp) {
  for (const auto &[pid, fdToFobj] : exp.inode->getAppFdMap()) {
    ...
    for (const auto &[fd, obj] : fdToFobj) {
      fdmap[fd] = exp.inode->i_no;
    }
  }
}
...
FsProcWorkerServant::FsProcWorkerServant(
    int w, CurBlkDev *d, int shmBaseOffset,
    std::vector<std::tuple<int, int, double, double>> &appConfigs,
    std::atomic_bool *workerRunning, PerWorkerLoadStatsSR *stats)
    : FsProcWorker(w, d, shmBaseOffset, appConfigs, workerRunning, stats) {
...
void FsProcWorkerServant::workerRunLoop() {
  SCHED_LOG_NOTICE("Worker %d starts to run", wid);
...
void FsProcWorker::blockingFlushBufferOnExit() {
  ...
  *ctx = 0;
  ...
  SPDLOG_DEBUG("wid:{} All buffers successfully flushed to disk ~~~", getWid());
}
...
bool FsProcWorkerServant::recvFileInodeSplitMsg(WorkerCommMessage *msg) {
  ...
  // install data block buffers
  fileManager->installDataBlockBufferSlot(inodePtr, msg->dataBlocks);

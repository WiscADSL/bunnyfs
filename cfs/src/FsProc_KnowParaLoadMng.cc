...
void KPLoadManager::loadManagerRunner() {
  pthread_setname_np(pthread_self(), "LoadManager");
  ...
  uint64_t lm_start_ts = PlatformLab::PerfUtils::Cycles::rdtscp();
  while (running_) {
    std::this_thread::sleep_for(
        std::chrono::microseconds(policy_ptr->GetEvalWindowUs()));
    [[maybe_unused]] uint64_t cur_nano =
        PlatformLab::PerfUtils::Cycles::toNanoseconds(
            PlatformLab::PerfUtils::Cycles::rdtscp() - lm_start_ts);
    eval_round_id_++;
    ...
    // read data
    for (auto wid : wids) {
      auto ts = PlatformLab::PerfUtils::Cycles::rdtscp();
  ...
  while (running_ && !core_has_msg_sent_.empty()) {
    ...
    switch (cur_type) {
      ...
      case FsProcMessageType::kLM_JoinAllAck: {
        ...
        auto cur_nano = PlatformLab::PerfUtils::Cycles::toNanoseconds(

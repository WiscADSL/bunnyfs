...
std::pair<uint64_t, uint64_t> Timer::Pause(bool record) {
  ...
  time_accumulated += PlatformLab::PerfUtils::Cycles::toNanoseconds(time_elapse);
  ...
  if (record) {
    ...
    started = false;
    return {PlatformLab::PerfUtils::Cycles::toNanoseconds(start_absolute),
            PlatformLab::PerfUtils::Cycles::toNanoseconds(end_absolute)};
  } else {
    started = false;
    return {0, 0};
  }

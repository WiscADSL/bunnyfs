...
void FileMng::ReassignmentOp::OwnerExportThroughPrimary(
  ...
  if (!exported) {
    SPDLOG_ERROR("Failed to export inode {}", ino);
    throw std::runtime_error("Failed to export inode");
  }
...
void FileMng::ReassignmentOp::ProcessInodeReassignmentReq(FileMng *mng,
  ...
  switch (irop.type) {
    ...
    case 1: {
      ...
      if (irop.curOwner == irop.newOwner) {  // no migration is necessary
        irop.ret = 0;
        mng->fsWorker_->submitFsReqCompletion(req);
        return;
      }
      ...
      auto oldOwnerCallback = [](FileMng *mng, const Ctx *reassignmentCtx,
        ...
        if (reassignmentCtx->state ==
          ...
          int ino = req->copPtr->op.inodeReassignment.inode;
          req->getApp()->EraseIno(ino);
      ...
      auto new_ctx = new NewOwnerCtx{
          .pid = app->getPid(),
          .tids = req->getApp()->GetTidsForIno(irop.inode),
          .ino = irop.inode,
      };

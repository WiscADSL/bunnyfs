...
int FsProcServerCoreAllocator::waitForAllWorkersAckRebalance() {
  ...
  while (numLeft > 0 && running_readonly_) {
    ...
    if (validMessage) {
      ...
      int wid = *fromWidCtx;
      [[maybe_unused]] auto widIt = ackedWid.find(wid);
...
void LoadManager::loadManagerRunner() {
  ...
  uint64_t start_ts = PlatformLab::PerfUtils::Cycles::rdtscp();
  while (running) {
    ...
    uint64_t cur_nano = PlatformLab::PerfUtils::Cycles::toNanoseconds(
        PlatformLab::PerfUtils::Cycles::rdtscp() - start_ts);

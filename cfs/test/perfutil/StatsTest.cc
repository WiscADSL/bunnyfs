...
void test1(TimerType tt, const std::string &msg) {
  ...
  for (int i = 0; i < opNum; i++) {
    if (tt == TimerType::RDTSC) {
      startTs = PlatformLab::PerfUtils::Cycles::rdtsc();
    ...
    if (tt == TimerType::RDTSC) {
      endTs = PlatformLab::PerfUtils::Cycles::rdtsc();
    } else if (tt == TimerType::GET_TIME_OF_DAY) {
      endTs = tap_ustime();
    } else {
...
void test3(int opNum) {
  ...
  start = PlatformLab::PerfUtils::Cycles::rdtsc();
  for (int i = 0; i < opNum; i++) {
    emulateOpFunc();
  }
  finish = PlatformLab::PerfUtils::Cycles::rdtsc();
  fprintf(stdout, " diff s:%f\n", PlatformLab::PerfUtils::Cycles::toSeconds(finish - start));
}
...
void testMachineTick() {
  fprintf(stdout, "1000 cycle to ns:%lu\n",
          PlatformLab::PerfUtils::Cycles::toNanoseconds(1000));
}

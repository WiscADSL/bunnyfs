...
// control cache hit rate; currently only effective with randread
static uint64_t FLAGS_working_set_size = std::numeric_limits<uint64_t>::max();
...
static double FLAGS_zipf_theta = 0;
...
class ZipfGtor {
 ...
 public:
  ZipfGtor(uint64_t n, double theta) : n_(n), theta_(theta), buffer_() {
    zeta_2_theta_ = zeta(2, theta);
    denom_ = zeta(n, theta);
    srand48_r(537, &buffer_);
  }

  uint64_t Zipf() {
    double alpha = 1 / (1 - theta_);
    double zetan = denom_;
    double eta = (1 - pow(2.0 / n_, 1 - theta_)) / (1 - zeta_2_theta_ / zetan);
    double u;
    drand48_r(&buffer_, &u);
    double uz = u * zetan;
    if (uz < 1) return 1;
    if (uz < 1 + pow(0.5, theta_)) return 2;
    return 1 + (uint64_t)(n_ * pow(eta * u - eta + 1, alpha));
  }
};
...
class Stats {
 ...
 public:
  ...
  void Start() {
    ...
    start_ = PlatformLab::PerfUtils::Cycles::toNanoseconds(g_env->NowCycles());
  ...
  void Stop() {
    if (!stopped_) {
      finish_ =
          PlatformLab::PerfUtils::Cycles::toNanoseconds(g_env->NowCycles());
...
struct ThreadState {
  ...
  ThreadState(int index, int cid, int aid, Slice p)
      : tid(index),
        ...
        crc_arr(nullptr),
        fileSize(0),
        zipf_gtor(FLAGS_working_set_size, FLAGS_zipf_theta) {}
  ...
  int inline DynamicDoBenchmarkBySegment(
    ...
    while (tk->segment_idx < FLAGS_num_segment) {
      ...
      uint64_t cur_start_ts_diff_us =
          PlatformLab::PerfUtils::Cycles::toMicroseconds(cur_ts -
                                                         tk->seg_start_ts);
 ...
 private:
  ...
  void DoRead(ThreadState *thread, bool seq) {
    ...
    if (FLAGS_rand_no_overlap) {
      ...
      max_req_num = FLAGS_working_set_size / FLAGS_rw_align_bytes;
    ...
    if (FLAGS_rw_align_bytes != 0) {
      if (FLAGS_rand_no_overlap) {
        ...
        for (uint64_t i = 0; i < max_req_num; i++) {
          off_vec[i] = (FLAGS_rw_align_bytes * i) % FLAGS_working_set_size;
        }
        ...
        std::uniform_int_distribution<uint64_t> distr(0, 
          FLAGS_working_set_size - 1);
        for (uint64_t i = 0; i < max_req_num; i++) {
          uint64_t off;
          if (FLAGS_zipf_theta < 1e-6)
            off = distr(generator);
          else
            off = thread->zipf_gtor.Zipf();
          off = (off / FLAGS_rw_align_bytes) * FLAGS_rw_align_bytes; // align
          assert(off < FLAGS_working_set_size);
          off_vec[i] = off;
        }
      }
    }
    ...
    for (int i = 0; i < numop_; i++) {
      ...
      if (seq) {
        ...
        if (FLAGS_rw_align_bytes != 0) {
          ...
          cur_off = thread->rand.Next() % FLAGS_working_set_size;
        }
        if (FLAGS_block_no >= 0) {
...
int main(int argc, char **argv) {
  for (int i = 1; i < argc; i++) {
    ...
    } else if (sscanf(argv[i], "--working-set=%lu%c", &uln, &junk) == 1) {
      FLAGS_working_set_size = uln;
  ...
  FLAGS_working_set_size = \
    std::min<uint64_t>(FLAGS_working_set_size, FLAGS_max_file_size);

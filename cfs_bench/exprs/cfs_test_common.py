...
def start_fs_proc(
    ...
    fs_cmd = (
        ...
        f"-w {num_workers} "
        f"-a {num_app_proc} "
        f"-c {','.join(map(str, core_ids))} "
        f"-e {exit_signal_file_name} "
        f"-f {fsp_config_name} "
        f"-d {config_name} "
    )
    ...
    if log_name is None:
        ...
        with open(log_name, "w") as f:
            f.write(fs_cmd)
            f.write("\n===\n")
        p_fs = run(f"{fs_cmd} >> {log_name}", async_=True, env=env)
    ...
    # For compatibility, use num_app_proc if not set
    if num_app_threads is None:
        num_app_threads = num_app_proc
    # If some apps have multiple threads, must ensure all threads are ready
    start_bench_coordinator(num_app_proc=num_app_threads)
...
def start_mtfsp(
    ...
    p_fs, exit_signal_file_name, real_ready_fname, fs_cmd = start_fs_proc(
        num_fsp_worker,
        num_app_proc,
        shm_offset=fsp_shmkeys, ready_fname=ready_fname,
        num_app_threads=num_app_threads, log_name=log_name,

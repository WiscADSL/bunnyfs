...
if [ -z "$AE_REPO_URL" ]; then
	export AE_REPO_URL='https://github.com/WiscADSL/uFS-sched.git'
fi
if [ -z "$AE_BRANCH" ]; then
...
export AE_REPO_DIR="$AE_WORK_DIR/uFS-sched"
...
function ae-init-install() {
	...
	add-if-not-exist 'export CFS_ROOT_DIR="${HOME}/workspace/uFS-sched"' ~/.ae_env.sh
